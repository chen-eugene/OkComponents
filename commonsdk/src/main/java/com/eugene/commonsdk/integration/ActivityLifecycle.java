...
import com.eugene.commonsdk.base.service.ActivityDelegateImpl;
import com.eugene.commonsdk.base.service.IActivity;
import com.eugene.commonsdk.base.service.IActivityDelegate;
import com.eugene.commonsdk.base.service.IFragmentDelegate;
...
import static com.eugene.commonsdk.base.service.IActivityDelegateKt.ACTIVITY_DELEGATE;
...
public class ActivityLifecycle implements Application.ActivityLifecycleCallbacks {
    ...
    private void registerFragmentCallbacks(Activity activity) {
        ...
        if (activity instanceof FragmentActivity && useFragment) {
            ...
            if (mExtras.containsKey(IntelligentCache.getKeyOfKeep(IConfigModule.class.getName()))) {
                List<IConfigModule> modules = (List<IConfigModule>) mExtras.get(IntelligentCache.getKeyOfKeep(IConfigModule.class.getName()));
                for (IConfigModule module : modules) {
                    module.injectFragmentLifecycle(mApplication, mFragmentLifecycles.get());
                }
                mExtras.remove(IntelligentCache.getKeyOfKeep(IConfigModule.class.getName()));
            }
    ...
    private IActivityDelegate fetchActivityDelegate(Activity activity) {
        ...
        if (activity instanceof IActivity) {
            Cache<String, Object> cache = getCacheFromActivity((IActivity) activity);
            activityDelegate = (IActivityDelegate) cache.get(IntelligentCache.getKeyOfKeep(ACTIVITY_DELEGATE));
        }
