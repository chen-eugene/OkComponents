package com.eugene.commonsdk.base.delegate
...
import com.eugene.commonsdk.di.component.DaggerSdkComponent
...
class AppDelegate(private val context: Context?) : IApp, IAppLifecycle {
    ...
    private var mArmComponent: SdkComponent? = null
    ...
    override fun onCreate(base: Application) {
        ...
        this.mArmComponent = DaggerSdkComponent.builder()
                .application(base)
                .armConfigModule(getGlobalConfigModule(base, mModules))
                .build()
        mArmComponent?.inject(this)
        ...
        mArmComponent?.extras()?.put(IntelligentCache.getKeyOfKeep(IConfigModule::class.java.name), mModules)
        ...
        mComponentCallback = AppComponentCallbacks(base, mArmComponent)
        //注册回掉: 内存紧张时释放部分内存
    ...
    /**
     * 将 {@link ArmComponent} 返回出去, 供其它地方使用, {@link ArmComponent} 接口中声明的方法返回的实例, 在 {@link #getAppComponent()} 拿到对象后都可以直接使用
     *
     * @return ArmComponent
     * @see ArmsUtils#obtainAppComponentFromContext(Context) 可直接获取 {@link ArmComponent}
     */
    override fun getArmComponent(): SdkComponent? {
        Preconditions.checkNotNull(
                mArmComponent,
                "%s == null, first call %s#onCreate(Application) in %s#onCreate()",
                SdkComponent::class.java.name, this::class.java.name,
                mApplication?.javaClass?.name ?: Application::class.java.name
        )

        return mArmComponent
    }
     ...
     */
    private fun getGlobalConfigModule(context: Context, modules: List<IConfigModule>?): RepositoryConfigModule? {
        val builder: RepositoryConfigModule.Builder = RepositoryConfigModule.Builder({})
...
class AppComponentCallbacks(
        ...
        private val armComponent: SdkComponent?
) : ComponentCallbacks2 {
